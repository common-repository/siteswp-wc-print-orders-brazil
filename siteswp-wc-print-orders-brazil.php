...
class SWP_Print_Orders {
    ...
    /**
     * Context para ser enviado com os logs em wc_get_logger
     * 
     */
    protected $context = ['source' => 'siteswp-wc-print-orders-brazil'];
    ...
    function setup(){
        ...
        // sinalizar em log caso as extensões de imagem não estejam instaladas
        if( !extension_loaded('gd') ){
            wc_get_logger()->warning( 'Módulo gd não instalado', $this->context );

            if( !extension_loaded('imagick') ){
                wc_get_logger()->warning( 'Módulo imagick não instalado', $this->context );
            }
        }
    ...
    protected function print_order( $order ){
        ...
        if( !empty($address['cep']) ){
            ...
            // forçar uso de GD para criação das imagens
            $generator->useGd();

            try{
                $barcode_image = $generator->getBarcode($address['cep'], $generator::TYPE_CODE_128, $this->barcode_config['width_factor'], $this->barcode_config['height']);
                $barcode = base64_encode($barcode_image);
                wc_get_logger()->info( sprintf('Código de barras gerado para o pedido %s (%s): %s', $order->get_id(), esc_html($address['cep']), $barcode), $this->context ); 
            }
            catch( Exception $e ){
                wc_get_logger()->error( $e->getMessage(), $this->context ); 
            }
        }

        if( empty($barcode) ){
            wc_get_logger()->warning( sprintf('Código de barras vazio para o pedido %s', $order->get_id()), $this->context );
        }
    ...
    protected function set_invoice( $order ){
        ...
        // Definir se o cliente possui CPF ou CNPJ
        $cpf_cnpj = ($order->get_meta('_billing_persontype') == 2) ? $order->get_meta('_billing_cnpj') : $order->get_meta('_billing_cpf');
        ...
        <div class="invoice-page">
            ...
            <table class="invoice-client" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td class="document"><strong class="label">CPF/CNPJ:</strong> <span class="value" title="CPF"><?php echo esc_html( $cpf_cnpj ); ?></span></td>
                </tr>
                <tr>
